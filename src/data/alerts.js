	...
	},
	{
		...
		request:
		{
			...
			[
				...
				{ name: "metric_name", type: "string — enum depends on signal_type, see schema.js alert_rules.metric_name", required: true },
		...
		constraints: {
			...
			[
				...
				"Rule triggers when the row's aggregate — computed per signal_type/metric_name/aggregation/window_seconds, see schema.js alert_rules — is >= threshold at evaluation time — fixed direction, no comparison-operator choice. All three signal types (ep-ingest-logs/-metrics/-traces in internal.js) now evaluate rules the same windowed-query way; the only difference between them is which ClickHouse table is queried and which metric_name/aggregation vocabulary applies",
